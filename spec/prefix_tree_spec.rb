...
RSpec.describe do
  ...
  describe '#write file' do
    it "write words in the file" do
      expect(@tree.write_file('spec/test_files/write')).to eq(nil)
    end
    it "the file contain words from tree?" do
      words_arr = []
      File.open('spec/test_files/write', 'r') { |file| file.each_line { |word| words_arr << word.chomp } }
  ...
  describe '#write.zip file' do
    it "write words in the zip file" do
      expect(@tree.write_zip('spec/test_files/write.zip')).to eq(nil)
    end
    it "zip file contain words from tree?" do
      words_arr = []
      Zip::File.open('spec/test_files/write.zip') { |zip_f| zip_f.read('write').each_line { |word| words_arr << word.chomp } }
      expect(words_arr).to eq(@tree.list)
    end
  end
